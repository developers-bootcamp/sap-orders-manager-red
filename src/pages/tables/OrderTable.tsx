...
const boldheader: any = (params: any) => {
  return <strong style={{ color: "grey" }}>{params.field}</strong>;
};
...
const OrderTable = (props: any) => {
  ...
  const getAllOrdersAsync = () => {
    ...
    getOrders(firstPaginationModel.page, filters).then((res) => {
  ...
  const getRows = (orders: IOrder[]) => {
    ...
    orders.forEach((e, index) => {
      if (
        ...
        currentRows.push({
          id: "null",
          price: e.totalAmount?.toFixed(2) + "" + e.currency,
          status: "null",
          customer: "null",
          products: "null",
          createDate: "null",
